...
5. Enter `ksh /fs/usb1/install.sh`
   - **If you encouter VM freezes ot I/O errors while copying:** try to create a .iso (via `AnyToIso Lite` as example) from the contents of the toolbox and mount the iso via CD drive. You can then run `ksh /fs/cd0/install.sh`!
